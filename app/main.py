from __future__ import annotations

import json

from fastapi import FastAPI, HTTPException
from fastapi.responses import HTMLResponse, JSONResponse

from app.render import render_api_summary, render_evidence, render_outcomes, render_overview
from app.services.ledger_service import build_service

app = FastAPI(
    title="Learner Intervention Ledger",
    version="0.1.0",
    description=(
        "EdTech intervention ledger for outreach history, escalation tracking, and student support outcome analysis."
    ),
)

service = build_service()


@app.get("/", response_class=HTMLResponse)
def overview() -> str:
    return render_overview()


@app.get("/outcomes", response_class=HTMLResponse)
def outcomes_page() -> str:
    return render_outcomes()


@app.get("/evidence", response_class=HTMLResponse)
def evidence_page() -> str:
    return render_evidence()


@app.get("/api-summary", response_class=HTMLResponse)
def api_summary_page() -> str:
    return render_api_summary()


@app.get("/api/dashboard/summary")
def dashboard_summary() -> dict:
    return service.summary()


@app.get("/api/interventions")
def interventions() -> list[dict]:
    return service.events()


@app.get("/api/lanes")
def lanes() -> list[dict]:
    return service.lane_breakdown()


@app.get("/api/outcomes")
def outcomes() -> list[dict]:
    return service.outcome_breakdown()


@app.get("/api/interventions/{intervention_id}")
def intervention(intervention_id: str) -> dict:
    value = service.intervention(intervention_id)
    if value is None:
        raise HTTPException(status_code=404, detail="Intervention not found")
    return value


@app.get("/api/sample")
def sample() -> dict:
    return service.sample_payload()


@app.get("/openapi.json")
def openapi_spec() -> JSONResponse:
    return JSONResponse(json.loads(json.dumps(app.openapi())))


if __name__ == "__main__":
    import os
    import uvicorn

    port = int(os.environ.get("PORT", "4741"))
    uvicorn.run("app.main:app", host="127.0.0.1", port=port, reload=False)
